...
def getFrequencyDict(sequence): ##Fait un dictionnaire avec un mots
    """
...
def getWordScore(word): ## Donne le score d'un mot
    """
...
class Hand(object):
    def __init__(self, handSize, initialHandDict = None): ##The__init__ is building a hand
...
##        return dict((k, v) for k, v in self.handDict.items() if v > 0) ## NECESSARY??
        return 


    def containsLetters(self, letters): ## OK, NOT TESTED
        """
    ...
    def __eq__(self, other):
        ...
        ## TO FIX
        if self.handDict == other:
            return True
        
        return False

        
    def __str__(self): ## ALREADY DONE
        """
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        if self.points > other:
            return 1
        elif self.points < other:
            return -1
        elif self.points == other:
            return 0
        else:
            return "There was an error with comparison"
